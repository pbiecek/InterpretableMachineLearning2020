...
dtrain_xgb = model.matrix(recid_violent ~ . - person_id - 1, data = data$train)
label = as.numeric(data$train$recid_violent) - 1
...
model_violent = xgboost(params = grid_search_list[[best_ind]], 
...
xgb_explain_violent = DALEX::explain(model_violent, data = data$test,
                                     y = data$test$recid_violent == 1, 
                                     type = 'classification', label = 'XGB Violent',
                                     predict_function = xgb_predict)
```

```{r}
xgb_perf = DALEX::model_performance(xgb_explain_violent, cutoff = 0.5)
xgb_perf
```

```{r}
save(xgb_explain_violent, model_violent, data, xlevs, file = 'xgb_all_violent.RData')
```
